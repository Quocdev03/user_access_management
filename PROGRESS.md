...
| `internal/handler/auth_handler.go` | `Register`, `VerifyEmail`, `Login` (IP & UserAgent), `RefreshToken`, `Logout` |
| `internal/router/router.go` | `POST /api/v1/auth/register`, `POST /api/v1/auth/verify-email`, `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh-token`, `POST /api/v1/auth/logout` (protected) |
...
1. **UC-25 Email Service** (SMTP) → thay mock bằng mail thật sử dụng cấu hình SMTP có sẵn.
2. **UC-07 Forgot Password** + **UC-08 Change Password** → hoàn thiện các luồng mật khẩu.
3. **UC-28 Audit Logging** → ghi lại nhật ký đăng nhập và hành động của user.
